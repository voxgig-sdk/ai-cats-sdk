...
class TrainingDirectTest < Minitest::Test
  def test_direct_list_training
    setup = training_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-training", setup[:live] ? "live" : "unit")
    if _should_skip
...
def training_direct_setup(mockres)
