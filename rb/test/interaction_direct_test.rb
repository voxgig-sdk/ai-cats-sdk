...
class InteractionDirectTest < Minitest::Test
  def test_direct_list_interaction
    setup = interaction_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-interaction", setup[:live] ? "live" : "unit")
    if _should_skip
...
def interaction_direct_setup(mockres)
