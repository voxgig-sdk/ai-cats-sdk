...
class TrainingEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = AiCatsSDK::test($seed, null);
        $seen = iterator_to_array($base->Training(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = AiCatsConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = AiCatsSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Training(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = training_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create", "list"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "training." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set AI_CATS_TEST_TRAINING_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // CREATE
        $training_ref01_ent = $client->Training(null);
        ...
        $training_ref01_data = Helpers::to_map(is_object($training_ref01_data_result) && method_exists($training_ref01_data_result, 'data_get') ? $training_ref01_data_result->data_get() : $training_ref01_data_result);
...
function training_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/training/TrainingTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["training01", "training02", "training03"] as $k) {
    ...
    $entid_env_raw = getenv("AI_CATS_TEST_TRAINING_ENTID");
