...
class FtCtrl extends AiCatsControl
{
...
class FtClient
{
    ...
    private AiCatsUtility $utility;

    public function __construct(AiCatsUtility $utility, array $options)
    {
    ...
    public function get_utility(): AiCatsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AiCatsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AiCatsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AiCatsConfig::make_feature($name);
    ...
    private static function buildUrl(AiCatsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AiCatsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AiCatsResponse($fetched);
        }
    ...
    private function fail(AiCatsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AiCatsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
