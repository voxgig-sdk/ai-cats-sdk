...
class AiCatsTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            AiCatsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = AiCatsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = AiCatsTestRunner.getenv("PROJECTENV_TEST_LIVE")
        override = AiCatsTestRunner.getenv("PROJECTENV_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = AiCatsTestRunner.getenv(key)
        ...
        explain = AiCatsTestRunner.getenv("PROJECTENV_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if AiCatsTestRunner._test_control is not None:
            return AiCatsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                AiCatsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            AiCatsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = AiCatsTestRunner.load_test_control()
...
def load_env_local():
    AiCatsTestRunner.load_env_local()


def env_override(m):
    return AiCatsTestRunner.env_override(m)


def entity_data(v):
    return AiCatsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return AiCatsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return AiCatsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return AiCatsTestRunner.load_test_control()


def live_client_options():
    return AiCatsTestRunner.live_client_options()


def live_delay_ms():
    return AiCatsTestRunner.live_delay_ms()
