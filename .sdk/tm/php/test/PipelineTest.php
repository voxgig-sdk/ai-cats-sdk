...
{
    private static function utility(): AiCatsUtility
    {
        return new AiCatsUtility();
    }
    ...
    private static function ctx(array $over = []): AiCatsContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new AiCatsContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new AiCatsOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof AiCatsError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new AiCatsOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = AiCatsMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new AiCatsOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = AiCatsMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new AiCatsOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = AiCatsMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AiCatsSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = AiCatsMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new AiCatsResponse([]),
            'result' => new AiCatsResult([]),
        ]);
        [, $err] = AiCatsMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new AiCatsSpec(['step' => 's']),
            'response' => new AiCatsResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new AiCatsResult([]),
        ]);
        [, $err] = AiCatsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new AiCatsSpec(['step' => 's']),
            'response' => new AiCatsResponse(self::resp(200, ['v' => 1])),
            'result' => new AiCatsResult([]),
        ]);
        [, $err] = AiCatsMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new AiCatsSpec(['step' => 's']),
            'response' => new AiCatsResponse(self::resp(200, ['v' => 2])),
            'result' => new AiCatsResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        AiCatsMakeResponse::call($ctx);
    ...
    {
        $preset = new AiCatsResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new AiCatsSpec([]),
            'response' => new AiCatsResponse([]),
            'result' => new AiCatsResult([]),
        ...
        [$got, $err] = AiCatsMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new AiCatsResult([])]);
        [, $err] = AiCatsMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new AiCatsSpec([]), 'result' => null]);
        [, $err] = AiCatsMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AiCatsSpec(['step' => 's']),
            'result' => new AiCatsResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new AiCatsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AiCatsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AiCatsSpec(['step' => 's']),
            'result' => new AiCatsResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new AiCatsOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AiCatsMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AiCatsResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new AiCatsSpec([]),
            'result' => new AiCatsResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = AiCatsMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AiCatsMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new AiCatsError('boom', 'boom');
        $utility->fetcher = function (AiCatsContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AiCatsSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = AiCatsMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (AiCatsContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = AiCatsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(AiCatsResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (AiCatsContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AiCatsSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = AiCatsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new AiCatsResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new AiCatsSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = AiCatsMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = AiCatsMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = AiCatsMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new AiCatsResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, AiCatsDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new AiCatsResult(['ok' => false])]);
        $this->expectException(AiCatsError::class);
        AiCatsDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new AiCatsResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', AiCatsMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new AiCatsBaseFeature();
        $b = new AiCatsBaseFeature();
        AiCatsFeatureAdd::call($ctx, $a);
        AiCatsFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): AiCatsBaseFeature
    {
        $f = new AiCatsBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        AiCatsFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): AiCatsContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new AiCatsSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AiCatsPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new AiCatsResponse(['headers' => null]),
            'result' => new AiCatsResult([]),
        ]);
        AiCatsResultHeaders::call($ctx);
