...
//    only - a typed AiCatsContext there would make every ctx
...
final class AiCatsOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new AiCatsContext($clean, null);
    ...
    public static function err_from_map($m): ?AiCatsError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new AiCatsError(is_string($code) ? $code : '', $msg);
    }
