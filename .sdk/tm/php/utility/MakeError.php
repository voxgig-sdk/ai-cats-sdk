...
class AiCatsMakeError
{
    public static function call(?AiCatsContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new AiCatsContext([], null);
        }
        $op = $ctx->op ?? new AiCatsOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new AiCatsResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof AiCatsError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new AiCatsError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof AiCatsError) {
