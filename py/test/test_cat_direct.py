...
class TestCatDirect:
    ...
    def test_should_direct_load_cat(self):
        setup = _cat_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-cat", "live" if setup["live"] else "unit")
        if _skip:
