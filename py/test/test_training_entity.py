...
class TestTrainingEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Training(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Training(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set AI_CATS_TEST_TRAINING_ENTID JSON to run live")
        client = setup["client"]
        ...
        training_ref01_data = helpers.to_map(runner.entity_data(training_ref01_ent.create(training_ref01_data, None)))
...
def _training_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("AI_CATS_TEST_TRAINING_ENTID"))
