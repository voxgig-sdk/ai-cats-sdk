    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set AI_CATS_TEST_CAT_ENTID JSON to run live")
        client = setup["client"]
        ...
        cat_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(cat_ref01_data_dt0_loaded))
...
def _cat_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("AI_CATS_TEST_CAT_ENTID"))
