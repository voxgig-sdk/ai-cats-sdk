...
The Python SDK for the AiCats API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Cat(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.AiCats(None).load(
    {"id": "test01"}, None
)
