...
from aicats_sdk.aicats_types import (
    Interaction,
...
class InteractionEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return InteractionEntity(self._client, opts)
    ...
    def match_get(self) -> Interaction:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Interaction]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Interaction().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: InteractionCreateData, ctrl=None) -> Interaction:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
