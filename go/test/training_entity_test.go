...
func TestTrainingEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Training(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Training(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		trainingRef01Data = core.ToMapAny(entityData(trainingRef01DataResult))
...
func trainingBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("AI_CATS_TEST_TRAINING_ENTID")
	...
	idmapResolved := core.ToMapAny(env["AI_CATS_TEST_TRAINING_ENTID"])
