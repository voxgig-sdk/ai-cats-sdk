...
func TestInteractionEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Interaction(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Interaction(nil).Stream("list", nil, nil) {
...
func interactionBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("AI_CATS_TEST_INTERACTION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["AI_CATS_TEST_INTERACTION_ENTID"])
