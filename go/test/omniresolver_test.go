...
import (
	...
	omni "github.com/voxgig-sdk/ai-cats-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AiCatsError); is {
				out := map[string]any{
					"name":    "AiCatsError",
...
func omniCtx(arg any, client *sdk.AiCatsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AiCatsSDK); is {
			client = live
