...
describe("CatEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Cat(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = cat_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "cat." .. _op, _live and "live" or "unit")
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set AI_CATS_TEST_CAT_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local cat_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.cat")))
    local cat_ref01_data = nil
    if #cat_ref01_data_raw > 0 then
      cat_ref01_data = helpers.to_map(cat_ref01_data_raw[1][2])
    end

    -- LOAD
    local cat_ref01_ent = client:Cat(nil)
    local cat_ref01_match_dt0 = {
...
function cat_basic_setup(extra)
  ...
  local entity_data_file = _test_dir .. "../../.sdk/test/entity/cat/CatTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read cat test data: " .. entity_data_file)
  end
  ...
  local entid_env_raw = os.getenv("AI_CATS_TEST_CAT_ENTID")
