...
describe("InteractionEntity", function()
  it("should create instance", function()
    ...
    local ent = testsdk:Interaction(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Interaction(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Interaction(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = interaction_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"create", "list"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "interaction." .. _op, _live and "live" or "unit")
    ...
    -- CREATE
    local interaction_ref01_ent = client:Interaction(nil)
    local interaction_ref01_data = helpers.to_map(vs.getprop(
    ...
    -- LIST
    local interaction_ref01_match = {}

    local interaction_ref01_list_result, err = interaction_ref01_ent:list(interaction_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(interaction_ref01_list_result)
...
function interaction_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/interaction/InteractionTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read interaction test data: " .. entity_data_file)
  end
