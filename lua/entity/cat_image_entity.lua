...
local CatImageEntity = {}
CatImageEntity.__index = CatImageEntity


function CatImageEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CatImageEntity)
  self._name = "cat_image"
  self._client = client
...
function CatImageEntity:get_name()
  return self._name
end


function CatImageEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CatImageEntity.new(self._client, opts)
end
...
function CatImageEntity:mark_deleted()
  self._deleted = true
end


function CatImageEntity:deleted()
  return true == self._deleted
end


function CatImageEntity:data_set(args)
  if args ~= nil then
...
function CatImageEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CatImageEntity:match_set(args)
  if args ~= nil then
...
function CatImageEntity:match_get()
...
function CatImageEntity:stream(action, args, callopts)
...
---@param reqmatch CatImageLoadMatch
---@param ctrl? table
---@return CatImage
---@return string? err
function CatImageEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function CatImageEntity:_run_op(ctx, post_done)
  local utility = self._utility
