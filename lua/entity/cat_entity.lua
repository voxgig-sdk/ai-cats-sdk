...
local CatEntity = {}
CatEntity.__index = CatEntity


function CatEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CatEntity)
  self._name = "cat"
  self._client = client
...
function CatEntity:get_name()
  return self._name
end


function CatEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CatEntity.new(self._client, opts)
end
...
function CatEntity:mark_deleted()
  self._deleted = true
end


function CatEntity:deleted()
  return true == self._deleted
end


function CatEntity:data_set(args)
  if args ~= nil then
...
function CatEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CatEntity:match_set(args)
  if args ~= nil then
...
function CatEntity:stream(action, args, callopts)
...
function CatEntity:_run_op(ctx, post_done)
  local utility = self._utility
