-- AiCats SDK Interaction entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local InteractionEntity = {}
InteractionEntity.__index = InteractionEntity


function InteractionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, InteractionEntity)
...
function InteractionEntity:get_name()
  return self._name
end


function InteractionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return InteractionEntity.new(self._client, opts)
end
...
function InteractionEntity:mark_deleted()
  self._deleted = true
end


function InteractionEntity:deleted()
  return true == self._deleted
end


function InteractionEntity:data_set(args)
  if args ~= nil then
...
function InteractionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function InteractionEntity:match_set(args)
  if args ~= nil then
...
function InteractionEntity:match_get()
...
function InteractionEntity:stream(action, args, callopts)
...
function InteractionEntity:_run_op(ctx, post_done)
  local utility = self._utility
