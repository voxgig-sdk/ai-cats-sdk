-- AiCats SDK Training entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local TrainingEntity = {}
TrainingEntity.__index = TrainingEntity


function TrainingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TrainingEntity)
  self._name = "training"
  self._client = client
...
function TrainingEntity:get_name()
  return self._name
end


function TrainingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TrainingEntity.new(self._client, opts)
end
...
function TrainingEntity:mark_deleted()
  self._deleted = true
end


function TrainingEntity:deleted()
  return true == self._deleted
end


function TrainingEntity:data_set(args)
  if args ~= nil then
...
function TrainingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TrainingEntity:match_set(args)
  if args ~= nil then
...
function TrainingEntity:match_get()
...
function TrainingEntity:stream(action, args, callopts)
...
---@param reqmatch TrainingListMatch
---@param ctrl? table
---@return Training[]
---@return string? err
function TrainingEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata TrainingCreateData
---@param ctrl? table
---@return Training
---@return string? err
function TrainingEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function TrainingEntity:_run_op(ctx, post_done)
  local utility = self._utility
