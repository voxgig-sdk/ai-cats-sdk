...
The Lua SDK for the AiCats API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Cat(nil):load({ id = "example_id" }, nil)
...
local result, err = client:AiCats(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── ai-cats_sdk.lua    -- Main SDK module
...
The main module (`ai-cats_sdk`) exports the SDK constructor
