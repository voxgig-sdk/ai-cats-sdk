...
[MCP](https://modelcontextprotocol.io) server exposing the AiCats SDK as
two agent tools — `ai-cats_list` and `ai-cats_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ai-cats-mcp)
make build

# 2. Provide credentials via the environment
export AI_CATS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user ai-cats \
  -- /absolute/path/to/ai-cats-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./ai-cats-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// ai-cats_list: first page of records
{ "entity": "interaction" }
{ "entity": "interaction", "query": { } }

// ai-cats_load: one record by id
{ "entity": "cat", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ai-cats-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ai-cats \
     -- "$PWD"/dist/*/ai-cats-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ai-cats_list` and `ai-cats_load` tools now appear
   in new sessions. Ask the agent to *"list interaction using ai-cats"*
   and it calls `ai-cats_list` with `{"entity":"interaction"}`.
...
```sh
export AI_CATS_APIKEY=sk_live_xxx            # API key
export AI_CATS_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `ai-cats_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ai-cats_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `AI_CATS_APIKEY` | API key sent with every request. |
| `AI_CATS_BASE` | Optional override of the API base URL. |

### Entities

The 5 entities valid as the `entity` argument:

cat | cat_image | health | interaction | training

### Smoke test via HTTP (raw JSON-RPC)

```sh
./ai-cats-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ai-cats_load","arguments":{"entity":"cat","query":{"id":1}}}}'
```
