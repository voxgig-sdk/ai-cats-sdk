...
type Args struct {
	Entity string         `json:"entity" jsonschema:"cat | cat_image | health | interaction | training"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.AiCatsSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "ai-cats_list",
		Description: "List records from AiCats. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "ai-cats_load",
		Description: "Load a single record from AiCats. " +
...
func runOp(client *sdk.AiCatsSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.AiCatsSDK, name string) (sdk.AiCatsEntity, error) {
	switch strings.ToLower(name) {
	case "cat":
		return client.Cat(nil), nil
	case "cat_image":
		return client.CatImage(nil), nil
	case "health":
		return client.Health(nil), nil
	case "interaction":
		return client.Interaction(nil), nil
	case "training":
		return client.Training(nil), nil
