...
describe('CatImageDirect', async () => {
  ...
  test('direct-load-cat_image', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-cat_image', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['AI_CATS_TEST_CAT_IMAGE_ENTID']
