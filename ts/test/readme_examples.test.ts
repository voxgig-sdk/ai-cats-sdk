...
//     (\`new AiCatsSDK(...)\` / \`AiCatsSDK.test(...)\` become
//     \`AiCatsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"cat":{"test01":{"id":"test01"}},"cat_image":{"test01":{"id":"test01"}},"health":{"test01":{"id":"test01"}},"interaction":{"test01":{"id":"test01"}},"training":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AiCatsSDK(...)\` and any
// \`AiCatsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AiCatsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AiCatsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
