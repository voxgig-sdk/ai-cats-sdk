...
boru-driven command-line client **and** interactive REPL for the AiCats
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ai-cats-cli)
make build
...
./ai-cats-cli load 1 cat            # {id:1} shorthand
./ai-cats-cli load '{id:1}' cat       # explicit match map

# 5. Override the API base URL for a single call
AI_CATS_BASE=https://api.example.com ./ai-cats-cli load 1 cat
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ai-cats-cli
   ```
...
```sh
./ai-cats-cli load 1 cat          # scalar shorthand for {id:1}
./ai-cats-cli load '{id:1}' cat     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `ai-cats>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `cat`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ai-cats-cli`. |
...
(`./ai-cats-cli <expr>`) and interactively (the REPL), and expressions compose the
