// Command AiCats-cli is a CLI + REPL for the AiCats SDK,
...
const prompt = "ai-cats"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "cat cat_image health interaction training"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      AI_CATS_APIKEY (api key), AI_CATS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: AI_CATS_APIKEY carries the API key and
	// AI_CATS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("AI_CATS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("AI_CATS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewAiCatsSDK(opts)
